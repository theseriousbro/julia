...
| `Base.broadcast_indices(::StyleA, A)` | Declaration of the indices of `A` for broadcasting purposes (for AbstractArrays, defaults to `indices(A)`) |
...
For `AbstractArray` types, defining a BroadcastStyle supercedes the fallback choice,
...
yields another `SparseVecStyle`, that it's combination with a 2-dimensional array
