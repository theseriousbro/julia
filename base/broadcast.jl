...
@inline function broadcast(f, ::DefaultArrayStyle, ::Type{Bool}, inds::Indices, As...)
    dest = Base.similar(BitArray, inds)
    broadcast!(f, dest, As...)
end
...
function broadcast_nonleaf(f, s::DefaultArrayStyle, ::Type{ElType}, shape::Indices, As...) where ElType
    nargs = length(As)
    iter = CartesianRange(shape)
    if isempty(iter)
