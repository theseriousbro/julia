...
function is_self_quoting(@nospecialize(x))
    return isa(x,Number) || isa(x,AbstractString) || isa(x,Tuple) || isa(x,Type) || isa(x,Char) || x === nothing ||
        isa(x,Builtin) || isa(x,IntrinsicFunction)
end
...
function inline_expr(e::Expr, sv::InferenceState, stmts::Vector{Any}, boundscheck::Symbol)
    if e.head === :call
...
function inline_call(e::Expr, sv::InferenceState, stmts::Vector{Any}, boundscheck::Symbol)
    eargs = e.args
    if length(eargs) < 1
        return e
    end
        ...
        if f === _apply
            ...
            for i = 3:na
                ...
                elseif isa(t, DataType) && t.name === Tuple.name && !isvatuple(t) &&
                    ...
                    ntp = length(tp)::Int
                    if isa(aarg,SSAValue) && any(p->(p === DataType || p === UnionAll || p === Union), tp)
                        ...
                        if def !== nothing
                            ...
                            if isa(defex, Expr) && is_known_call(defex, tuple, sv.src, sv.mod)
                                ...
                                for j = 1:ntp
                                    ...
                                    if iskindtype(tp[j]) && specific_type ⊑ tp[j]
                                        tp[j] = specific_type
                                    end
                                end
                            end
...
function get_replacement(table::ObjectIdDict, var::Union{SlotNumber, SSAValue}, @nospecialize(init),
    ...
    elseif isa(init, Number) || init === () || init === nothing || isa(init, Type) || isa(init, Char) || isa(init, IntrinsicFunction) || isa(init, Builtin)
        return init
