...
(define (valid-ir-return? e)
  ...
  (or (valid-ir-rvalue? 'return e) (and (pair? e) (memq (car e) '(new lambda)))))
  ...
  (let ((code '())            ;; statements (emitted in reverse order)
    ...
    (define (emit-return x)
      ...
      (define (actually-return x)
        (let ((tmp (if (valid-ir-return? x) #f (make-ssavalue))))
          (if tmp (emit `(= ,tmp ,x)))
          (emit `(return ,(or tmp x)))))
      (if (> handler-level 0)
          (let ((tmp (cond ((simple-atom? x) #f)
            ...
            (if finally-handler
                ...
                (begin (emit `(leave ,handler-level))
                       (actually-return (converted (or tmp x)))))
            (or tmp x))
          (actually-return (converted x))))
